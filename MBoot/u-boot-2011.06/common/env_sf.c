...
void env_relocate_spec (void)
{
    ...
    extern void update_env(const char *s, const char *env);
    int ret=0;
    void* pEnvBuf = malloc(CONFIG_ENV_SIZE);
    if(pEnvBuf==NULL) return;
...
int spi_init (void)
{
    ...
    #endif
    printf("%s: cfg_env_offset: 0x%lx, at %d\n", __func__, (unsigned long)cfg_env_offset, __LINE__); //cyber
