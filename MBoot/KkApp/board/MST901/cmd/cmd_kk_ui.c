...
#define KK_UI_GWIN_WIDTH              720
#define KK_UI_GWIN_HEIGHT             576
...
int kk_ui_create_osdWin(void)
{
	...
	snprintf(cmd, cmd_size, "osd_create 720 576");
	run_command("panel_init", 0);
	if(run_command("destroy_logo", 0))	
	{
		KK_UI_DBG(printf("destroy logo error>>>\n"));
		return -1;
	}
	if(!run_command(cmd, 0))
	{
		KK_UI_DBG(printf("create osd win ok>>>\n"));
		if(cmd)
    			free(cmd);
		return 0;
	}
	else
	{
		if(cmd)
    			free(cmd);
		KK_UI_DBG(printf("create osd win error>>>\n"));
		return -1;
	}
	return 0;
}
...
{
	const int cmd_size = 60;
	char *cmd= NULL;
	cmd =(char*)malloc(cmd_size);
	memset( cmd, 0, cmd_size );
	snprintf(cmd, cmd_size, "draw_rect %d %d %d %d 0x%x",dispX, dispY, dispWidth, dispHeiht, color);
	printf("cmd ====%s \n",cmd );
	if(!run_command(cmd, 0))
	{
		KK_UI_DBG(printf("draw rect ok..>>>\n"));
		if(cmd)
    			free(cmd);
		return 0;
	}
	else
	{
		if(cmd)
    			free(cmd);
		KK_UI_DBG(printf("draw rect error..>>>\n"));
		return -1;
	}
}
...
int kk_ui_draw_progress(unsigned short dispX, unsigned short dispY, unsigned short per, unsigned int color)
{
	...
	cmd =(char*)malloc(cmd_size);
	memset( cmd, 0, cmd_size );
	snprintf(cmd, cmd_size, "draw_progress %d %d 0x%x %d",dispX, dispY, color, per);
	printf("cmd ====%s \n",cmd );
	if(!run_command(cmd, 0))
	{
		KK_UI_DBG(printf("draw progress ok>>>\n"));
		if(cmd)
    			free(cmd);
		return 0;
	}
	else
	{
		if(cmd)
    			free(cmd);
		KK_UI_DBG(printf("draw progress error..>>>\n"));
		return -1;
	}
}
...
{
	const int cmd_size = 250;
	char *cmd= NULL;
	cmd =(char*)malloc(cmd_size);
	memset( cmd, 0, cmd_size );
	snprintf(cmd, cmd_size, "draw_string %d %d 0x%x %d %s",dispX, dispY, color, attrib, string);
	printf("cmd ====%s \n",cmd );
	if(!run_command(cmd, 0))
	{
		KK_UI_DBG(printf("draw string ok>>>\n"));
		if(cmd)
    			free(cmd);
		return 0;
	}
...
{
	if(!run_command("osd_flush", 0))	
	{
		KK_UI_DBG(printf("osd flush ok..>>>\n"));
		return 0;
	}
	else
	{
		KK_UI_DBG(printf("osd flush error..>>>\n"));
		return -1;
	}
}
...
{
	if(!run_command("osd_destroy", 0))	
	{
		KK_UI_DBG(printf("osd destory ok..>>>\n"));
		return 0;
	}
	else
	{
		KK_UI_DBG(printf("osd destory error..>>>\n"));
		return -1;
	}
}
...
int do_kk_boot_draw_string(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	char cStr[202] = {0};
	unsigned short u16Total = 0;
	short i = 5;

	for (; i < argc; ++i)
	{
		unsigned short u16Len = strlen(argv[i]);
	...
	cStr[u16Total] = 0;
	ret = kk_ui_draw_string(dispX, dispY, color, attrib, cStr);
	return ret;
}
...
int do_kk_boot_destory_OsdWin (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	int ret = -1;
