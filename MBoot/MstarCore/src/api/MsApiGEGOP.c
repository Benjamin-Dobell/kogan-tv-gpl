...
void MsApiDisplay_Logo(GFX_DrawRect *rect,GFX_BufferInfo *src_info,GFX_BufferInfo *dst_info, MS_BOOL bHorStretch, MS_BOOL bVerStretch,MS_U16 u16DispX,MS_U16 u16DispY)
{
	...
	printf("[AutoTest][MBoot][Show Logo]\n");
...
void MsApiDrawJPG(GFX_BufferInfo src_info,GFX_Block Blk)
{
	...
	dst_info.u32ColorFmt = (GFX_Buffer_Format)E_MS_FMT_ARGB4444;
...
void MsApiDrawRect(GFX_Block* pBlk, GFX_RgbColor color)
{
    ...
    gfxDstBuf.u32ColorFmt = (GFX_Buffer_Format)E_MS_FMT_ARGB4444;
    ...
    gfxFillBlock.fmt = (GFX_Buffer_Format)E_MS_FMT_ARGB4444;
...
static MS_S16 findASCCharIndex(const char* pStrText)
{
    ...
    {
    	 //printf("pStrText[0] ======= %d\n", pStrText[0] );
	 //printf("ASC_CHAR_TABLE[%d] ======= %d\n", idx, ASC_CHAR_TABLE[idx] );		 
...
static MS_S16 findUniCharIndex(const char* pStrText)
{
    ...
    for (; idx < u16Len; idx++)
    {
    	// printf("idx ======= %d\n", idx );
    	 //printf("pStrText[1] ======= %d\n", pStrText[1] );
    	 //printf("UNI_CHAR_TABLE[idx++] ======= %d\n", UNI_CHAR_TABLE[idx+1] );	
	 //printf("pStrText[0] ======= %d\n", pStrText[0] );
	 //printf("UNI_CHAR_TABLE[idx] ======= %d\n", UNI_CHAR_TABLE[idx] );
	 if(UNI_CHAR_TABLE[idx] == pStrText[0])
	 {
	 	if(UNI_CHAR_TABLE[idx+1] == pStrText[1])
	 	{
	 		return idx >> 1;
	 	}
	 }
...
static MS_BOOL drawUniAndASCStrText(const char* pStrText, MS_U16 X, MS_U16 Y, GFX_RgbColor color, TextAttrib eTextAttrib)
{
    //printf("draw uni and asc str text!!>>\n");	
    switch (eTextAttrib)
    {
        case eTextAlignMiddle:
        {
    ...
    for(; i < u16Len; i += 2)
    {
        X += u16Offset;
	 MS_S16 s16Idx;	
	 if((MS_U8)pStrText[i] < 0xA0)
	 {
	 	printf("draw asc str text!!>>\n");
	 	s16Idx= findASCCharIndex(&pStrText[i]);
        	drawOneASCCharByIdx(s16Idx, X, Y, color);
        	u16Offset = (s16Idx < 0) ? SPACE_WIDTH : (CHAR_WIDTH_TABLE[s16Idx] + EACH_CHAR_INTERVAL);
	 }
	 else
	 {
	 	printf("draw uni str text!!>>\n");
	        s16Idx = findUniCharIndex(&pStrText[i]);
	        drawOneUniCharByIdx(s16Idx, X, Y, color);
	        u16Offset = (s16Idx < 0) ? SPACE_WIDTH : (ONE_CHAR_WIDTH + EACH_CHAR_INTERVAL);
	 }
    }
...
{
     printf("pStrText[0] ======= %d\n", pStrText[0]);		
    ((MS_U8)pStrText[0] < 0xA0) ? drawASCStrText(pStrText, X, Y, color, eTextAttrib) : drawUniAndASCStrText(pStrText, X, Y, color, eTextAttrib);
    //drawUniAndASCStrText(pStrText, X, Y, color, eTextAttrib);//LI JIA MODIFIED
	return ;
}
void MsApiFlushCanvas2Screen(void)
{
    ...
    gfxSrcBuf.u32ColorFmt = (GFX_Buffer_Format)E_MS_FMT_ARGB4444;
