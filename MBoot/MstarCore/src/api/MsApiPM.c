...
{
    // set boot mode flag to env
    char *envstr=NULL;
    char envname[16], envvalue[8];
    const char *envnamestr = "51OnRam";

#if ((ENABLE_BOOTING_FROM_EXT_SPI_WITH_PM51==1)||(ENABLE_BOOTING_FROM_OTP_WITH_PM51==1))
    const char *configstr = "1";
#else
    const char *configstr = "0";
#endif
    envstr = getenv ((char*)envnamestr);

    if((envstr==NULL) || (strncmp(envstr, configstr, 1) != 0))
    {
        memset(envname, 0, sizeof(envname));
        memset(envvalue, 0, sizeof(envvalue));
        snprintf(envname, sizeof(envname)-1, "%s", envnamestr);
        snprintf(envvalue, sizeof(envvalue)-1, "%s", configstr);
