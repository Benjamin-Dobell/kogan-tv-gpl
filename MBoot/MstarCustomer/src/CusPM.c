...
EN_POWER_ON_MODE ePowerMode=EN_POWER_DC_BOOT;
    ...
    if(MDrv_WDT_IsReset() && strcmp(getenv("dc_poweroff"), "1") == 0)
    {
    ...
    if ( EN_POWER_AC_BOOT == ePowerMode)
    {
		...
		if(getenv("game_mode") == NULL)
		{
		    setenv("game_mode", "disable");
            saveenv();
		}
        if(getenv("factory_burnin_mode") == NULL)
        {
            setenv("factory_burnin_mode", "0");
            saveenv();
        }
        if (( strcmp(getenv("factory_poweron_mode"), "secondary") == 0 ||    //Secondary, AC on will always enter standby mode
           ( strcmp(getenv("factory_poweron_mode"), "memory") == 0 && strcmp(getenv("dc_poweroff"), "1") == 0))
#if (CONFIG_MSTAR_PM_SWIR)
           && MDrv_PM_STR_CheckFactoryPowerOnMode_Second(1)
#endif
			&& strcmp(getenv("factory_burnin_mode"), "0") == 0
			&& (strcmp(getenv("upgrade_mode"), "usb") != 0 && strcmp(getenv("upgrade_mode"), "mboot") != 0)
           ) //Memory, DC off -> AC off -> AC on -> Standby
        {
    ...
    /*else //DC power on
    {
         setenv("dc_poweroff", "0");//clear DC flag
         saveenv();
    }*/
}
