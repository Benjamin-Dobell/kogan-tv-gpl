...
int vfs_read(void* addr,char* filedir,unsigned int offset,unsigned int size )
{
    ...
    if(strncmp(filedir, "/customercfg/",13)==0)
    {
        printf("mingwu_mboot----------vfs_read--=000=\n");
        filedir +=13;//skip /customercfg
        mmc_volume="customercfg";
        snprintf(buffer, CMD_BUF, "ext4partload mmc 0:0xA %x %s %x %x", (unsigned int)addr, filedir, offset,size);
    }
    else
    {
        printf("mingwu_mboot----------vfs_read--=111=\n");
