...
static EN_UPDATE_MODE s_UpdateMode = EN_UPDATE_MODE_USB;
...
//-------------------------------------------------------------------------------------------------
#if(ENABLE_MODULE_DONT_OVERWRITE==1)
int do_force_overwirte_init(void)
{
    UBOOT_TRACE("IN\n");
...
int do_dont_overwrite_init(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    char *env_state=getenv(ENV_FORCEUPGRADE);
    if(env_state != NULL)
    {
         if(simple_strtoul(env_state, NULL, 16) > 0)
         {
             pdont_overwrite_partition[dont_overwrite_count]=malloc(strlen("reset")+1);
             if(pdont_overwrite_partition[dont_overwrite_count]==NULL){
                 UBOOT_ERROR("malloc for 'reset' fail\n");
                 return -1;
             }
             memset(pdont_overwrite_partition[dont_overwrite_count],0,strlen("reset")+1);
             strncpy(pdont_overwrite_partition[dont_overwrite_count],"reset", strlen("reset")+1);
             dont_overwrite_count++;
         }
    }
...
int check_script_crc(char * upgradeFile, char *scriptBuf)
{
    ...
    //u32FileSize-(u32FileSize&0x1FF)-0x200 ???
    // (u32FileSize&0x1FF) : make sure offset alignmet 512 bytes
    // -0x200 : make sure length is over 512 bytes.
    #define TEMP_SIZE ((u32FileSize&0x1FF)+0x200)
...
int do_net_miu(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    int ret=0;
    UBOOT_TRACE("IN\n");
    buffer=(char *)malloc(MIU_SECRIPT_SIZE);
    if(buffer==NULL)
    {
        UBOOT_ERROR("no memory for miu setting command string!!\n");
        return -1;
    }
    ...
    if (argc > 1)
    {
        snprintf(Cmd_buffer, sizeof(Cmd_buffer), "tftp 0x%x %s", (U32)buffer, argv[1]);
        UBOOT_DEBUG("cmd=%s\n",Cmd_buffer);
    }
    else
    {
    ...
    {
        UBOOT_ERROR("read miu script file failed !!\n");
        free(buffer);
        return -1;
    }
    ...
    while ((next_line = get_script_next_line(&script_buf)) != NULL)
    {
        ...
        if(strncmp(temp,"setenv",6)==0)
        {
            memset(Cmd_buffer, 0, sizeof(Cmd_buffer));
            snprintf(Cmd_buffer, sizeof(Cmd_buffer), "%s", next_line);
        }
        else
        {
            memset(Cmd_buffer, 0, sizeof(Cmd_buffer));
            //If we enable  CONFIG_BACKUP_PARTITION , we will increase some commands, like setKL, setUBI..etc
            //So if we use "set" to replace "setenv", mboot will confused.
            //snprintf(Cmd_buffer, sizeof(Cmd_buffer), "set %s", next_line);
            snprintf(Cmd_buffer, sizeof(Cmd_buffer), "setenv %s", next_line);
        }
        UBOOT_DEBUG("cmd: %s\n", Cmd_buffer);
        ret=run_command(Cmd_buffer, 0);
        if(ret==-1)
        {
            free(buffer);
