...
static long set_fatent(fsdata *mydata, __u32 entry, __u32 value)
{
    ...
    FAT_DPRINT("set_fatent:value: %d, buffer: %d\n", value, ((__u32 *)bufptr)[offset]);
    return 0;
}
...
put_fatent(fsdata *mydata, __u32 entry, __u32 free, int num)
{
    ...
    if(entry == free) {
        ...
        if(set_fatent(mydata, entry, 0xffffffff) < 0)
            return -1;
...
//old
    ...
    while(1) {
        ...
        if(set_fatent(mydata, free, 0xffffffff) < 0)
            return -1;
         ...
         int dols)
{
    ...
    mydata->entnum = ALIGN(bs.total_sect - bs.reserved - bs.fat32_length - 1, bs.cluster_size) / bs.cluster_size + 2;
    ...
    mydata->fatbuf = (__u8 *) &g_current_fatbuf;

    if(check_fatbuf(mydata) < 0){
        free(mydata);
        return -1;
    }
