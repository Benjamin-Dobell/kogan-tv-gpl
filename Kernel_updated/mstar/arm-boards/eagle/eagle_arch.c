...
/*------------------------------------------------------------------------------
	...
	FILE NAME: arch/arm/mach-eagle/eagle_arch.c
...
-------------------------------------------------------------------------------*/
/* initialize eagle's IRQ */
extern void eagle_init_irq(void);
/* Add SD Card driver */
extern void __init eagle_add_device_mmc(short mmc_id);
extern void __init eagle_add_device_memstick(s16 memstick_id);
...
-------------------------------------------------------------------------------*/
/* eagle's system timer */
extern struct sys_timer eagle_timer;
extern struct sys_timer eagle_ptimer;
/*------------------------------------------------------------------------------
    Global Variable
-------------------------------------------------------------------------------*/
/* platform device for eagle_board_init() */
struct platform_device eagle_device_usbgadget =
{
...
/* platform device for eagle_board_init() */
static struct platform_device eagle_device_pmu =
{
	.name		= "arm-pmu",
...
/* platform device array for eagle_board_init() */
static struct platform_device *eagle_devices[] __initdata =
{
    &eagle_device_usbgadget,
    &eagle_device_pmu,
    &Mstar_emac_device,
    &Mstar_ir_device,
    &Mstar_nand_device,
...
static struct map_desc eagle_io_desc[] __initdata =
{
    ...
    {
		.virtual	= eagle_ITCM_VIRT,
		.pfn		= __phys_to_pfn(eagle_ITCM_PHYS),
		.length		= eagle_ITCM_SIZE,
		.type		= MT_DEVICE
    },
	/* Define DTCM' physcial and virtual addresses */
	{
		.virtual	= eagle_DTCM_VIRT,
		.pfn		= __phys_to_pfn(eagle_DTCM_PHYS),
		.length		= eagle_DTCM_SIZE,
		.type		= MT_DEVICE
	},
    /* Define MEM51' physcial and virtual addresses */
	{
		.virtual	= eagle_MEM51_VIRT,
		.pfn		= __phys_to_pfn(eagle_MEM51_PHY),
		.length		= eagle_MEM51_SIZE,
...
/*------------------------------------------------------------------------------
    Function: eagle_map_io

    Description:
        init eagle IO tables
    ...
    Remark:
        None.
-------------------------------------------------------------------------------*/
void __init eagle_map_io(void)
{
#ifdef CONFIG_HAVE_ARM_TWD
    twd_base=((void __iomem *)PERI_ADDRESS(0x16000600)); //private timer& watchdog base
#endif
    iotable_init(eagle_io_desc, ARRAY_SIZE(eagle_io_desc) );
}
...
static struct irqaction eagle_xiu_timeout_irq = {
    .name = "Eagle Timeout IRQ",
    .flags = IRQF_TIMER | IRQF_IRQPOLL | IRQF_DISABLED,
    .handler = (irq_handler_t) eagle_xiu_timeout_interrupt,

};

/*------------------------------------------------------------------------------
    Function: eagle_board_init

    Description:
        eagle board init function
    Input: (The arguments were used by caller to input data.)
        None.
    Output: (The arguments were used by caller to receive data.)
        None.
...
static void __init eagle_board_init(void)
{
    #ifdef CONFIG_CACHE_L2X0
    unsigned int val = 0;
    #endif

    platform_add_devices(eagle_devices, ARRAY_SIZE(eagle_devices));

    #if defined(CONFIG_USB_EHCI_HCD) || defined(CONFIG_USB_EHCI_HCD_MODULE)
        Mstar_ehc_platform_init();
    #endif

    #if defined(CONFIG_MEMSTICK_EAGLE_MS)
        eagle_add_device_memstick(0);
    #endif
    ...
    setup_irq(E_FIQ_XIU_TIMEOUT , &eagle_xiu_timeout_irq);
...
void Chip_Clean_Cache_Range(unsigned long u32Addr, unsigned long u32Size)
{
    ...
    //Clean L1
    dmac_map_area((unsigned long *) u32Addr, u32Size, 1);
...
void Chip_Inv_Cache_Range(unsigned long u32Addr, unsigned long u32Size)
{
    ...
    if (outer_cache.is_enable()) //check if L2 is enabled
    {
        if(!virt_addr_valid(u32Addr) || !virt_addr_valid(u32Addr+ u32Size - 1))
        {
           printk("Chip_Inv_Cache_Range: Input VA can't be converted to PA\n");
        }
        else
        {
            //Inv L2 by range
...
#endif
    //Inv L1
    dmac_map_area((unsigned long *) u32Addr, u32Size, 2);
...
void Chip_Clean_Cache_Range_VA_PA(unsigned long u32VAddr,unsigned long u32PAddr,unsigned long u32Size)
{
    ...
    //Clean L1
    dmac_map_area((unsigned long *) u32VAddr, u32Size, 1);
...
#endif
    //Inv L1
    dmac_map_area((unsigned long *) u32VAddr, u32Size, 2);
...
//#endif
    .map_io = eagle_map_io,
    .init_irq = eagle_init_irq,
    .init_machine = eagle_board_init,
    .timer = &eagle_timer,
MACHINE_END
