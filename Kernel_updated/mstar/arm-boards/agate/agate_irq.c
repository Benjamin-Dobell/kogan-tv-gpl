...
void agate_irq_ack(unsigned int irq) {
    unsigned short tmp;
    //printk(KERN_WARNING "agate_irq_ack(irq=%d)\n",irq);
        if(irq <16)
	{
	    tmp = (0x01 << irq);
...
/* Mask IRQ/FIQ */
void agate_irq_mask(unsigned int irq) {
    unsigned short tmp;
...
/* Un-Mask IRQ/FIQ */
void agate_irq_unmask(unsigned int irq) {
   unsigned short tmp;
...
void __init agate_init_irq(void) 
{
	...
	for(irq=0;irq<NR_IRQS;irq++){
		set_irq_chip(irq, &agate_irq_chip);
		//set_irq_chip(31, &agate_irq_chip);
