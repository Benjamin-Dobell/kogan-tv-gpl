...
#define REG_PM_MISC_BASE            (0xFD00000 + (0x2E00 << 1))
...
static inline void arch_reset(char mode, const char *cmd)
{
    ...
    //printk("machine_restart after %d milliseconds\n",BOOT_DELAY_MILLISECOND); 
    // fire reset for global chip, active by password (8'h79)
    // 1. set to default first
    //*(volatile unsigned long*)(REG_PM_MISC_BASE + 0xB8) = 0xff;    // reg_top_sw_rst
    // 2. active by password (8'h79)
    *(volatile unsigned int *)(REG_PM_MISC_BASE + 0xB8) = 0x79;    // reg_top_sw_rst     
 
#if 0 
    // set WDT count down timer  
    *(volatile unsigned int *)(REG_WDT_BASE + 0x10) = BOOT_DELAY_MILLISECOND & 0x0000FFFF;               
    *(volatile unsigned int *)(REG_WDT_BASE + 0x14) = BOOT_DELAY_MILLISECOND >> 16;               
#endif 
}
