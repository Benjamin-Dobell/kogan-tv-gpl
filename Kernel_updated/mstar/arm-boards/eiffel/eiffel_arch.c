...
/*------------------------------------------------------------------------------
	...
	FILE NAME: arch/arm/mach-eiffel/eiffel_arch.c
...
-------------------------------------------------------------------------------*/
/* initialize eiffel's IRQ */
extern void eiffel_init_irq(void);
/* Add SD Card driver */
extern void __init eiffel_add_device_mmc(short mmc_id);
extern void __init eiffel_add_device_memstick(s16 memstick_id);
...
-------------------------------------------------------------------------------*/
/* eiffel's system timer */
extern struct sys_timer eiffel_timer;
extern struct sys_timer eiffel_ptimer;
/*------------------------------------------------------------------------------
    Global Variable
-------------------------------------------------------------------------------*/
/* platform device for eiffel_board_init() */
struct platform_device eiffel_device_usbgadget =
{
...
/* platform device for eiffel_board_init() */
static struct platform_device eiffel_device_pmu =
{
	.name		= "arm-pmu",
...
/* platform device array for eiffel_board_init() */
static struct platform_device *eiffel_devices[] __initdata =
{
    &eiffel_device_usbgadget,
    &eiffel_device_pmu,
    &Mstar_emac_device,
    &Mstar_ir_device,
    &Mstar_nand_device,
...
static struct map_desc eiffel_io_desc[] __initdata =
{
    ...
    {
		.virtual	= eiffel_ITCM_VIRT,
		.pfn		= __phys_to_pfn(eiffel_ITCM_PHYS),
		.length		= eiffel_ITCM_SIZE,
		.type		= MT_DEVICE
    },
	/* Define DTCM' physcial and virtual addresses */
	{
		.virtual	= eiffel_DTCM_VIRT,
		.pfn		= __phys_to_pfn(eiffel_DTCM_PHYS),
		.length		= eiffel_DTCM_SIZE,
		.type		= MT_DEVICE
	},
    /* Define MEM51' physcial and virtual addresses */
	{
		.virtual	= eiffel_MEM51_VIRT,
		.pfn		= __phys_to_pfn(eiffel_MEM51_PHY),
		.length		= eiffel_MEM51_SIZE,
...
/*------------------------------------------------------------------------------
    Function: eiffel_map_io

    Description:
        init eiffel IO tables
    ...
    Remark:
        None.
-------------------------------------------------------------------------------*/
void __init eiffel_map_io(void)
{
#ifdef CONFIG_HAVE_ARM_TWD
    twd_base=((void __iomem *)PERI_ADDRESS(0x16000600)); //private timer& watchdog base
#endif
    iotable_init(eiffel_io_desc, ARRAY_SIZE(eiffel_io_desc) );
}
...
#ifdef CONFIG_MSTAR_EIFFEL_BD_FPGA
...
void eiffel_xiu_timeout_interrupt(int i,void * j)
{
...
static struct irqaction eiffel_xiu_timeout_irq = {
    .name = "Eiffel Timeout IRQ",
    .flags = IRQF_TIMER | IRQF_IRQPOLL | IRQF_DISABLED,
    .handler = (irq_handler_t) eiffel_xiu_timeout_interrupt,

};

/*------------------------------------------------------------------------------
    Function: eiffel_board_init

    Description:
        eiffel board init function
    Input: (The arguments were used by caller to input data.)
        None.
    Output: (The arguments were used by caller to receive data.)
        None.
...
static void __init eiffel_board_init(void)
{
    ...
    platform_add_devices(eiffel_devices, ARRAY_SIZE(eiffel_devices));

#if defined(CONFIG_USB_EHCI_HCD) || defined(CONFIG_USB_EHCI_HCD_MODULE)
    Mstar_ehc_platform_init();
#endif

#if defined(CONFIG_MEMSTICK_EIFFEL_MS)
    eiffel_add_device_memstick(0);
#endif
    ...
    setup_irq(E_FIQ_XIU_TIMEOUT , &eiffel_xiu_timeout_irq);
...
void Chip_Clean_Cache_Range(unsigned long u32Addr, unsigned long u32Size)
{
    ...
    //Clean L1
    dmac_map_area((unsigned long *) u32Addr, (size_t) u32Size, 1);
...
#endif
    //Inv L1
    dmac_map_area((unsigned long *) u32Addr, (size_t) u32Size, 2);
...
void Chip_Flush_Cache_Range_VA_PA(unsigned long u32VAddr,unsigned long u32PAddr,unsigned long u32Size)
{
    ...
    //Clean & Invalid L1
    dmac_flush_range((unsigned long *) u32VAddr, (unsigned long *) (u32VAddr + u32Size));
...
void Chip_Clean_Cache_Range_VA_PA(unsigned long u32VAddr,unsigned long u32PAddr,unsigned long u32Size)
{
    ...
    //Clean L1
    dmac_map_area((unsigned long *) u32VAddr,(size_t) u32Size, 1);
...
#endif
    //Inv L1
    dmac_map_area((unsigned long *)u32VAddr, (size_t) u32Size, 2);
...
void Chip_Flush_Cache_All(void)
{
        ...
        smp_call_function(__cpuc_flush_kern_all, (void *) NULL, 1);
...
//#endif
    .map_io = eiffel_map_io,
    .init_irq = eiffel_init_irq,
    .init_machine = eiffel_board_init,
