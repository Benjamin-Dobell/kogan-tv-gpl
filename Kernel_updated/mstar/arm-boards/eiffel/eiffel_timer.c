...
{
#if defined(CONFIG_MSTAR_EIFFEL_BD_FPGA)
    return 12;
#else
...
static irqreturn_t eiffel_timer_interrupt(int irq, void *dev_id)
{
    timer_tick();

    //stop timer
    CLRREG16(EIFFEL_BASE_REG_TIMER0_PA, TIMER_TRIG);

    //set interval
    //interval = (CLOCK_TICK_RATE / HZ);
    OUTREG16(EIFFEL_BASE_REG_TIMER0_PA + ADDR_TIMER_MAX_LOW, (interval &0xffff));
    OUTREG16(EIFFEL_BASE_REG_TIMER0_PA + ADDR_TIMER_MAX_HIGH, (interval >>16));

    //trig timer0
    SETREG16(EIFFEL_BASE_REG_TIMER0_PA, TIMER_TRIG);

    return IRQ_HANDLED;
}

static struct irqaction eiffel_timer_irq =
{
    .name = "Eiffel Timer Tick",
    .flags = IRQF_TIMER | IRQF_IRQPOLL | IRQF_DISABLED,
    .handler = eiffel_timer_interrupt,
};
#endif
...
void __init eiffel_clocksource_init(unsigned int base)
{
...
void __init eiffel_clockevents_init(unsigned int base,unsigned int irq)
{
...
void __init eiffel_init_timer(void)
{

#ifdef CONFIG_MSTAR_EIFFEL_BD_FPGA
    GLB_TIMER_FREQ_KHZ= 6*1000 ;              // PERIPHCLK = CPU Clock / 2,
...
#ifdef CONFIG_GENERIC_CLOCKEVENTS
    eiffel_clocksource_init(EIFFEL_BASE_REG_TIMER1_PA);
    eiffel_clockevents_init(EIFFEL_BASE_REG_TIMER0_PA,E_FIQ_EXTIMER0);
#else
    setup_irq(E_FIQ_EXTIMER0 , &eiffel_timer_irq);

    //enable timer interrupt
    SETREG16(EIFFEL_BASE_REG_TIMER0_PA,TIMER_INTERRUPT);
...
struct sys_timer eiffel_timer =
{
    .init = eiffel_init_timer,
};
...
static irqreturn_t eiffel_ptimer_interrupt(int irq, void *dev_id)
{
    //unsigned int count;
    timer_tick();
...
static struct irqaction eiffel_ptimer_irq = {
    .name = "Eiffel PTimer Tick",
    .flags = IRQF_TIMER | IRQF_IRQPOLL | IRQF_DISABLED,
    .handler = eiffel_ptimer_interrupt,
};

void __init eiffel_init_ptimer(void)
{
    ...
    */
    ...
    //setup timer IRQ
    setup_irq(INT_ID_PTIMER, &eiffel_ptimer_irq);
...
struct sys_timer eiffel_ptimer =
{
    .init = eiffel_init_ptimer,
};
