...
U32  eMMCTest_SgW_MIU(U32 u32_eMMC_Addr)
{
    ...
    for(u32_i=0; u32_i< MAX_SCATTERLIST_COUNT; u32_i++)
    {
        ...
        if(u32_i ==0)
        {
            ...
            if(eMMC_ST_SUCCESS != u32_err)
            {
      	        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR,0,"eMMC Err: wait FIFOClk\n");
      	        eMMC_FCIE_ErrHandler_Stop();
      	        goto LABEL_END;
            }
        }
...
U32  eMMCTest_SgR_MIU(U32 u32_eMMC_Addr)
{
    ...
    if(eMMC_ST_SUCCESS != u32_err)
    {
		eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 0, "eMMC Err: CMD18 %Xh \n", u32_err);
		eMMC_FCIE_ErrHandler_Stop();
		goto LABEL_END;
    }
    else
    {
        ...
        for(u32_i=1; u32_i< MAX_SCATTERLIST_COUNT; u32_i++)
        {
            ...
            u32_err = eMMC_FCIE_WaitEvents(FCIE_MIE_EVENT, BIT_MIU_LAST_DONE, TIME_WAIT_n_BLK_END*(1+(u32_dmalen>>16)));
	 
    		if(u32_err!= eMMC_ST_SUCCESS)
            {
                eMMC_debug(eMMC_DEBUG_LEVEL_ERROR,0,"eMMC Err: r timeout \n");
   			    //eMMC_die("\n");
   			    g_eMMCDrv.u32_DrvFlag |= DRV_FLAG_ERROR_RETRY;
   			    break;
            }
...
U32 eMMC_IPVerify_Performance(void)
{
    ...
    eMMC_debug(eMMC_DEBUG_LEVEL_ERROR,0,"burst size: %Xh \n", 1<<eMMC_SECTOR_512BYTE_BITS);
	eMMC_hw_timer_start();
	u32_t0 = eMMC_hw_timer_tick();
	...
	switch(u32_SectorCnt)
	{
		...
		case 0x400*4 >> eMMC_SECTOR_512BYTE_BITS:
			u32_SectorCnt = 0x400*8 >> eMMC_SECTOR_512BYTE_BITS; //16KB
		    break;
		case 0x400*8 >> eMMC_SECTOR_512BYTE_BITS:
			u32_SectorCnt = 0x400*16 >> eMMC_SECTOR_512BYTE_BITS; //16KB
		    break;
		case 0x400*16 >> eMMC_SECTOR_512BYTE_BITS:
			u32_SectorCnt = 0x400*32 >> eMMC_SECTOR_512BYTE_BITS; //32KB
		    break;
		case 0x400*32 >> eMMC_SECTOR_512BYTE_BITS:
			u32_SectorCnt = 0x400*64 >> eMMC_SECTOR_512BYTE_BITS; //64KB
		    break;
		case 0x400*64 >> eMMC_SECTOR_512BYTE_BITS:
			u32_SectorCnt = 0x400*128 >> eMMC_SECTOR_512BYTE_BITS; //128KB
		    break;
	...
	eMMC_debug(eMMC_DEBUG_LEVEL_ERROR,0,"burst size: %u KB \n", u32_SectorCnt>>1);
	eMMC_hw_timer_start();
	u32_t0 = eMMC_hw_timer_tick();
