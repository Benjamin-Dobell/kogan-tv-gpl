...
void eMMC_FCIE_DumpRegisters(void)
{
	...
	eMMC_debug(eMMC_DEBUG_LEVEL, 1, "MIU side JobCnt: %Xh \n", REG_FCIE(FCIE_TR_BK_CNT));
	REG_FCIE_CLRBIT(FCIE_JOB_BL_CNT, BIT_SD_JOB_CNT_SRC_MIU);
	eMMC_debug(eMMC_DEBUG_LEVEL, 1, "Card side JobCnt: %Xh \n", REG_FCIE(FCIE_TR_BK_CNT));
...
U32 eMMC_LoadDDRTTable(void)
{
	...
	if(0 == u32_ChkSum && 
		g_eMMCDrv.DDRTable.Set[0].u8_Clk == g_eMMCDrv.DDRTable.Set[1].u8_Clk)
	{
...
{
	eMMC_clock_setting(g_eMMCDrv.DDRTable.Set[u8_DDRTIdx].u8_Clk);
...
U32 eMMC_CMD8_MIU(U8 *pu8_DataBuf)
{
	...
	eMMC_flush_data_cache_buffer((U32)pu8_DataBuf, eMMC_SECTOR_512BYTE);
	eMMC_FCIE_CLK_DIS();
	return u32_err;
}
...
U32 eMMC_CMD18_MIU(U32 u32_eMMCBlkAddr, U8 *pu8_DataBuf, U16 u16_BlkCnt)
{
	...
	U8  u8_retry_fifoclk=0, u8_retry_fcie=0, u8_retry_r1=0, u8_retry_cmd=0;
	U32 u32_dma_addr;
	U16 u16_BlkCnt_tmp, u16_BlkCnt_done=0, u16_BlkCnt_prev;
	...
	eMMC_FCIE_ClearEvents();
	u16_BlkCnt_tmp = u16_BlkCnt-u16_BlkCnt_done > BLK_BATCH_CNT ? BLK_BATCH_CNT : u16_BlkCnt-u16_BlkCnt_done;
	REG_FCIE_W(FCIE_JOB_BL_CNT, u16_BlkCnt_tmp);
    u32_dma_addr = eMMC_translate_DMA_address_Ex(
		(U32)pu8_DataBuf+(u16_BlkCnt_done<<eMMC_SECTOR_512BYTE_BITS), 
		eMMC_SECTOR_512BYTE*u16_BlkCnt_tmp);
	#if FICE_BYTE_MODE_ENABLE
	REG_FCIE_W(FCIE_SDIO_ADDR0, u32_dma_addr & 0xFFFF);
	REG_FCIE_W(FCIE_SDIO_ADDR1, u32_dma_addr >> 16);
	#else
	...
	while(1)
	{
		u16_BlkCnt_prev = u16_BlkCnt_tmp;
		u16_BlkCnt_done += u16_BlkCnt_tmp;
		//eMMC_debug(0,0,"%Xh %Xh %Xh \n", u16_BlkCnt_tmp, u16_BlkCnt_done, u16_BlkCnt);
		if(u16_BlkCnt_done < u16_BlkCnt)
		{
			u16_BlkCnt_tmp = u16_BlkCnt-u16_BlkCnt_done > BLK_BATCH_CNT ? BLK_BATCH_CNT : u16_BlkCnt-u16_BlkCnt_done;
			u32_dma_addr = eMMC_translate_DMA_address_Ex(
				(U32)pu8_DataBuf+(u16_BlkCnt_done<<eMMC_SECTOR_512BYTE_BITS), 
		...
		// --------------------------------------
	    u32_err = eMMC_FCIE_WaitEvents(FCIE_MIE_EVENT, 
	        BIT_MIU_LAST_DONE, TIME_WAIT_n_BLK_END*(1+(u16_BlkCnt_prev>>9)));
		...
		if(u16_BlkCnt_done == u16_BlkCnt)
		{
			// clean previous buffer from cache
		    eMMC_flush_data_cache_buffer(
				(U32)pu8_DataBuf+((u16_BlkCnt_done-u16_BlkCnt_prev)<<eMMC_SECTOR_512BYTE_BITS), 
				eMMC_SECTOR_512BYTE*u16_BlkCnt_prev);
