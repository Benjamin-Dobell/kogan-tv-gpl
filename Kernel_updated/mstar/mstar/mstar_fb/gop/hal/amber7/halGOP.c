...
void _HAL_GOP_SetBlending(U8 u8GOP, U8 u8win, U8 bEnable, U8 u8coef)
{
	...
	_HAL_GOP_Write16Reg(u32bankoff+GOP_4G_GWIN0_CTRL(u8win), (u8coef>>2)<<8, 0x3F00);
	_HAL_GOP_Write16Reg(u32bankoff+GOP_4G_GWIN_ALPHA01(u8win), (u8coef&0x3), 0x3);
}
void _HAL_GOP_SetGOPEnable2SC(U8 u8GOP, U8 bEnable)
{
    ...
    */
    ...
    else if (u8GOP== ((muxValue & (GOP_BIT3|GOP_BIT2))>>(GOP_MUX_SHIFT*1))) //enable mux1
    {
        _HAL_GOP_Write16Reg(GOP_SC_GOPEN, bEnable?(regval |0x1000):(regval & ~0x1000), 0x1000);
    }
    else if (u8GOP== ((muxValue & (GOP_BIT5|GOP_BIT4))>>(GOP_MUX_SHIFT*2))) //enable mux2
    {
        _HAL_GOP_Write16Reg(GOP_SC_GOPEN, bEnable?(regval |0x4000):(regval & ~0x4000), 0x4000);
    }
    else if (u8GOP== ((muxValue & (GOP_BIT7|GOP_BIT6))>>(GOP_MUX_SHIFT*3))) //enable mux3
    {
