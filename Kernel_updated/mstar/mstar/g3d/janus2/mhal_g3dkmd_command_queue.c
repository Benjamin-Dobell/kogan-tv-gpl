...
#if defined(CONFIG_ANDROID_PMEM) && defined(USING_PMEM)
...
#define MIU1_BUSADDRESS 0x60000000 
...
static int _find_dev_cb(struct device *dev, void *data)
{
    struct platform_device* device = container_of(dev, struct platform_device, dev);
    struct query_info_t* info = (struct query_info_t*)data;

    G3D_HAL_DBG("iterate bus device: name = %s, id = %d\n", device->name, device->id);
    if (info->name && strncmp(info->name, device->name, strlen(info->name)))
        return 0;
...
static int get_tex_channel_from_pmem1(void)
{
    struct android_pmem_platform_data *data = 0;
    ...
    bus_for_each_dev(&platform_bus_type, NULL, &query_info, &_find_dev_cb);
    if (query_info.dev == 0)
    {
        printk("platform device android_pmem (id=1) not found\n");
        return 0;
    }

    data = query_info.dev->dev.platform_data;
    G3D_HAL_DBG("name = %s, start = 0x%x, size = 0x%x\n", data->name, data->start, data->size);
    return ((data->start & MIU1_BUSADDRESS) ? 1 : 0);
}
#endif
...
#if defined(CONFIG_ANDROID_PMEM) && defined(USING_PMEM)
    tex_channel = get_tex_channel_from_pmem1();
#else
...
G3DKMD_BOOL _g3dkmd_SubmitCommands(const void* commands, unsigned int length, unsigned int* stamp)
{
    ...
    //        (unsigned long)(_g3dkCommandQueue.Size));
     __asm__ __volatile__ ("sync;"); //flush the pipe
    {
        //G3D_MMAP_BUFFER Buf;
