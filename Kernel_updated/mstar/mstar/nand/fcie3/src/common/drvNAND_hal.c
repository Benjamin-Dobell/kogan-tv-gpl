...
)
{
	...
	u32_DataDMAAddr = nand_translate_DMA_address_Ex((U32)pu8_DataBuf, pNandDrv->u16_PageByteCnt * u32_PageCnt, 0);
    #if IF_SPARE_AREA_DMA
	if (pu8_SpareBuf)
		u32_SpareDMAAddr = nand_translate_DMA_address_Ex((U32)pu8_SpareBuf, pNandDrv->u16_SpareByteCnt, 0);
	else
	{
		...
		u32_SpareDMAAddr = nand_translate_DMA_address_Ex((U32)pNandDrv->PlatCtx_t.pu8_PageSpareBuf, pNandDrv->u16_SpareByteCnt, 0);
	}
...
)
{
	...
	u32_DataDMAAddr = nand_translate_DMA_address_Ex((U32)pu8_DataBuf, pNandDrv->u16_PageByteCnt * u32_PageCnt, 1);
    #if IF_SPARE_AREA_DMA
	if (pu8_SpareBuf)
		u32_SpareDMAAddr = nand_translate_DMA_address_Ex((U32)pu8_SpareBuf, pNandDrv->u16_SpareByteCnt, 1);
	else
		u32_SpareDMAAddr = nand_translate_DMA_address_Ex((U32)pNandDrv->PlatCtx_t.pu8_PageSpareBuf, pNandDrv->u16_SpareByteCnt, 1);
	REG_WRITE_UINT16(NC_SPARE_DMA_ADR0, u32_SpareDMAAddr & 0xFFFF);
	REG_WRITE_UINT16(NC_SPARE_DMA_ADR1, u32_SpareDMAAddr >>16);
    #endif
...
//JC: dual core with L2 hardcore patch
  ...
  Chip_Flush_Cache_Range_VA_PA((U32)pu8_DataBuf,__pa(pu8_DataBuf),pNandDrv->u16_PageByteCnt * u32_PageCnt);
	...
	if (u16_Tmp & BIT_NC_ECC_FAIL)
	{
		...
		if( u8_RetryCnt_ECCFail < u8_MaxReadRetryCount)
		{
			...
			if(!pNandDrv->u8_RequireRandomizer)
			{
				...
				//	NC_Toshiba_ReadRetrySequence(u8_RetryCnt_ECCFail - 1);
				u8_RetryCnt_ECCFail++;
			}
			#if defined(FCIE_LFSR) && FCIE_LFSR
			else
		...
		#if defined(FCIE_LFSR) && FCIE_LFSR
		else if(u8_RetryCnt_ECCFail == u8_MaxReadRetryCount)
		{
	...
	if(NAND_DMA_PATCH_WAIT_TIME == u16_Tmp)
	{
		...
		{
			nand_debug(UNFD_DEBUG_LEVEL_ERROR, 1, "Soft reset over 50 times\n, stop!\n"); //If soft reset still can not solve this problem, show an alert and return a error
			NC_UNLOCK_FCIE();
...
U32  NC_ReadSectors_MTD(U32 u32_PhyRowIdx, U8 u8_SectorInPage, U8 *pu8_DataBuf, U8 *pu8_SpareBuf, U32 u32_SectorCnt)
{
	...
	if (u32_SectorCnt_Tmp)
	{
		...
		u32_DataDMAAddr = nand_translate_DMA_address_Ex((U32)pu8_DataBuf, pNandDrv->u16_SectorByteCnt*u32_SectorCnt_Tmp, 1);
		#if IF_SPARE_AREA_DMA
		if (pu8_SpareBuf)
			u32_SpareDMAAddr = nand_translate_DMA_address_Ex((U32)pu8_SpareBuf, pNandDrv->u16_SectorSpareByteCnt*u32_SectorCnt_Tmp, 1);
		else
			u32_SpareDMAAddr = nand_translate_DMA_address_Ex((U32)pNandDrv->PlatCtx_t.pu8_PageSpareBuf, pNandDrv->u16_SpareByteCnt, 1);
		REG_WRITE_UINT16(NC_SPARE_DMA_ADR0, u32_SpareDMAAddr & 0xFFFF);
		REG_WRITE_UINT16(NC_SPARE_DMA_ADR1, u32_SpareDMAAddr >>16);
...
)
{
	...
	u32_DataDMAAddr = nand_translate_DMA_address_Ex((U32)pu8_DataBuf, pNandDrv->u16_SectorByteCnt, 0);
    #if IF_SPARE_AREA_DMA
	if (pu8_SpareBuf)
		u32_SpareDMAAddr = nand_translate_DMA_address_Ex((U32)pu8_SpareBuf, pNandDrv->u16_SectorSpareByteCnt, 0);
	else
	{
		for (u16_Tmp=0; u16_Tmp < pNandDrv->u16_PageSectorCnt; u16_Tmp++)
			memset(pNandDrv->PlatCtx_t.pu8_PageSpareBuf + pNandDrv->u16_SectorSpareByteCnt * u16_Tmp,
				   0xFF, pNandDrv->u16_SectorSpareByteCnt - pNandDrv->u16_ECCCodeByteCnt);
		u32_SpareDMAAddr = nand_translate_DMA_address_Ex((U32)pNandDrv->PlatCtx_t.pu8_PageSpareBuf, pNandDrv->u16_SectorSpareByteCnt, 0);
	}
	REG_WRITE_UINT16(NC_SPARE_DMA_ADR0, u32_SpareDMAAddr & 0xFFFF);
	REG_WRITE_UINT16(NC_SPARE_DMA_ADR1, u32_SpareDMAAddr >>16);
    #endif
...
)
{
	...
	u32_DataDMAAddr = nand_translate_DMA_address_Ex((U32)pu8_DataBuf, (2 * pNandDrv->u16_PageByteCnt) * u32_PageCnt, 0);
    #if IF_SPARE_AREA_DMA
	if (pu8_SpareBuf)
		u32_SpareDMAAddr = nand_translate_DMA_address_Ex((U32)pu8_SpareBuf, pNandDrv->u16_SpareByteCnt, 0);
	else
	{
		...
		u32_SpareDMAAddr = nand_translate_DMA_address_Ex((U32)pNandDrv->PlatCtx_t.pu8_PageSpareBuf, 2 * pNandDrv->u16_SpareByteCnt, 0);
	}
	REG_WRITE_UINT16(NC_SPARE_DMA_ADR0, u32_SpareDMAAddr & 0xFFFF);
	REG_WRITE_UINT16(NC_SPARE_DMA_ADR1, u32_SpareDMAAddr >>16);
    #endif
	...
	REG_WRITE_UINT16(NC_SDIO_ADDR0, u32_DataDMAAddr & 0xFFFF);//>>MIU_BUS_WIDTH_BITS));
	REG_WRITE_UINT16(NC_SDIO_ADDR1, u32_DataDMAAddr >> 16);//(MIU_BUS_WIDTH_BITS+16)));
	REG_SET_BITS_UINT16(NC_MMA_PRI_REG, BIT_NC_DMA_DIR_W|BIT_NC_MIU_BURST_8);//BIT_NC_MIU_BURST_32);//BIT_NC_MIU_BURST_16);//
	if( (u32_Err=NC_waitFifoClkReady()) != UNFD_ST_SUCCESS)
	{
		#if 0==IF_IP_VERIFY
		NC_ResetFCIE();
...
)
{
	...
	u32_DataDMAAddr = nand_translate_DMA_address_Ex((U32)pu8_DataBuf, (2 * pNandDrv->u16_PageByteCnt) * u32_PageCnt, 1);
    #if IF_SPARE_AREA_DMA
	if (pu8_SpareBuf)
		u32_SpareDMAAddr = nand_translate_DMA_address_Ex((U32)pu8_SpareBuf, pNandDrv->u16_SpareByteCnt, 1);
	else
	...
	{
		REG_WRITE_UINT16(NC_AUXREG_DAT, (ACT_WAITRB << 8) | CMD_0x30);
		REG_WRITE_UINT16(NC_AUXREG_DAT, (CMD_0x00 << 8) | ACT_SER_DIN);
		REG_WRITE_UINT16(NC_AUXREG_DAT, (CMD_0x30 << 8) | u16_Tmp);
		REG_WRITE_UINT16(NC_AUXREG_DAT, (ACT_SER_DIN << 8) | ACT_WAITRB);
	}
	REG_WRITE_UINT16(NC_AUXREG_DAT, (ACT_BREAK << 8) | ACT_REPEAT);
	...
	if (NC_WaitComplete(BIT_NC_JOB_END|BIT_MMA_DATA_END, WAIT_READ_TIME * 2 *u32_PageCnt) ==
                (WAIT_READ_TIME * 2 *u32_PageCnt ))
	{
		nand_debug(UNFD_DEBUG_LEVEL_ERROR, 1, "Error: NC_ReadPages Timeout, ErrCode:%Xh \r\n", UNFD_ST_ERR_R_TIMEOUT);
		#if 0==IF_IP_VERIFY
		NC_ResetFCIE();
	...
	if (pu8_SpareBuf)
	{
		...
		#endif
		NC_GetCIFD(pu8_SpareBuf, 0, pNandDrv->u16_SpareByteCnt);
		memcpy((void *)(pu8_SpareBuf+pNandDrv->u16_SpareByteCnt),
			(const void *) pu8_SpareBuf, pNandDrv->u16_SpareByteCnt);
	}
	#endif
...
nand_redo:
	...
	{
		if(u8_RWDirect == 1)
		    u32_DataDMAAddr = nand_translate_DMA_address_Ex((U32)pu8_DataBuf, pNandDrv->u16_PageByteCnt * u8_plane * u32_PageCnt, 1 );
		else if(u8_RWDirect == 2)
			u32_DataDMAAddr = nand_translate_DMA_address_Ex((U32)pu8_DataBuf, pNandDrv->u16_PageByteCnt * u8_plane * u32_PageCnt, 0 );
		else
		{
