...
static u32 UART16550_READ(u8 byReadAddress)
{
    unsigned int    dwReadData = 0;
 
    if(byReadAddress <= 80) 
    {
        dwReadData = UART_REG(byReadAddress);
    
        if(byReadAddress == 0)
        {
            unsigned int    dwDataCounter = 0;
            
            keypool[keyidx ++] = dwReadData;
            keyidx &= UART_KEY_MASK;
            
            for(dwDataCounter = 0; dwDataCounter < UART_KEY_SIZE; dwDataCounter++)
            {
                if(keypool[(keyidx + dwDataCounter) & UART_KEY_MASK] != keyword[dwDataCounter])
                {
                    break;
                }
            }
            
            if(dwDataCounter >= UART_KEY_SIZE)
            {
                silent_state = (1 - silent_state);
                if(silent_state == 1)
                {
    				*((volatile unsigned int*)(0xBF000000 + (0x101E3E << 1))) |= 0x0100;
    		    }
            }
...
static void UART16550_WRITE(u8 byWriteAddress, u8 byWriteData)
{
    if(byWriteAddress > 80) //ERROR: Not supported
    {
        printk("W: %d\n",byWriteAddress);
        return;
    }
    else if(1 == silent_state || g_in_hotel_mode)
    {
        return ;
    }
    else
    {
        UART_REG(byWriteAddress) = byWriteData;
    }
}
