...
static int Titania3_EnableInterrupt (InterruptNum eIntNum)
{
    int bRet = 0;
...
static int Titania3_DisableInterrupt (InterruptNum eIntNum)
{
...
static unsigned int startup_Titania3_irq(unsigned int irq)
{
	//enable_lasat_irq(irq);
    Titania3_EnableInterrupt((InterruptNum)irq);
	return 0; /* never anything pending */
}

static void shutdown_Titania3_irq(unsigned int irq)
{
	//enable_lasat_irq(irq);
    Titania3_DisableInterrupt((InterruptNum)irq);
	//return 0; /* never anything pending */
}

#define enable_Titania3_irq startup_Titania3_irq
#define disable_Titania3_irq shutdown_Titania3_irq
#define mask_and_ack_Titania3_irq shutdown_Titania3_irq
#define end_Titania3_irq startup_Titania3_irq

static unsigned int startup_Titania3_fiq(unsigned int irq)
{
	//enable_lasat_irq(irq);
    Titania3_EnableInterrupt((InterruptNum)irq);
	return 0; /* never anything pending */
...
static void shutdown_Titania3_fiq(unsigned int irq)
{
	//enable_lasat_irq(irq);
    Titania3_DisableInterrupt((InterruptNum)irq);
	//return 0; /* never anything pending */
...
#define enable_Titania3_fiq startup_Titania3_fiq
#define disable_Titania3_fiq shutdown_Titania3_fiq
#define mask_and_ack_Titania3_fiq shutdown_Titania3_fiq
#define end_Titania3_fiq startup_Titania3_fiq

// Dean modify to match kernel 2.6.26
static struct hw_interrupt_type Titania3_irq_type = {
	.name = "Titania3 IRQ",
	.startup = startup_Titania3_irq,
	.shutdown = shutdown_Titania3_irq,
	.enable = (void (*)(unsigned int irq))enable_Titania3_irq,
	.disable = disable_Titania3_irq,
	.ack = mask_and_ack_Titania3_irq,
	.mask = disable_Titania3_irq,
	.mask_ack = disable_Titania3_irq,
	.unmask = (void (*)(unsigned int irq))enable_Titania3_irq,
	.eoi = (void (*)(unsigned int irq))enable_Titania3_irq,
	.end = (void (*)(unsigned int irq))end_Titania3_irq
};
static struct hw_interrupt_type Titania3_fiq_type = {
	.name = "Titania3 IRQ",
	.startup = startup_Titania3_fiq,
	.shutdown = shutdown_Titania3_fiq,
	.enable = (void (*)(unsigned int irq))enable_Titania3_fiq,
	.disable = disable_Titania3_fiq,
	.ack = mask_and_ack_Titania3_fiq,
	.mask = disable_Titania3_fiq,
	.mask_ack = disable_Titania3_fiq,
	.unmask = (void (*)(unsigned int irq))enable_Titania3_fiq,
	.eoi = (void (*)(unsigned int irq))enable_Titania3_fiq,
	.end = (void (*)(unsigned int irq))end_Titania3_fiq
};

// void Titania3_hw0_irqdispatch(struct pt_regs *regs)
void Titania3_hw0_irqdispatch(void)
{
...
// void Titania3_hw0_fiqdispatch(struct pt_regs *regs)
void Titania3_hw0_fiqdispatch(void)
{
    unsigned int        u16Reg = 0;
    unsigned int        u32Bit;
    InterruptNum        eIntNum;
...
asmlinkage void plat_irq_dispatch(void)
{
	...
	irq = irq_ffs(pending);
       ...
       //if(irq != 7)
            //printk("\nirq = %d\n", irq);
       if (irq == 2)
              Titania3_hw0_irqdispatch();
       else if (irq == 3)
              Titania3_hw0_fiqdispatch();
	else if (irq >= 0)
...
void __init arch_init_irq(void)
{
	...
	//mips_cpu_irq_init(0);
#if 0
	for (i = 0; i <= Titania3INT_END; i++) {
		irq_desc[i].status	= IRQ_DISABLED;
		irq_desc[i].action	= 0;
		irq_desc[i].depth	= 1;
		if ( i <64)
                irq_desc[i].chip    = &Titania3_irq_type;
		   //irq_desc[i].handler	= &Titania3_irq_type;
		else
                irq_desc[i].chip    = &Titania3_fiq_type;
		   //irq_desc[i].handler  = &Titania3_fiq_type;
	}
    ...
    if (cpu_has_vint) {
        ...
        set_vi_handler(2, Titania3_hw0_irqdispatch);
        set_vi_handler(3, Titania3_hw0_fiqdispatch);
    ...
    for (i = MSTAR_INT_BASE; i <= (TITANIA3INT_END+MSTAR_INT_BASE); i++) {
        if ( i <64+MSTAR_INT_BASE)
            set_irq_chip_and_handler(i, &Titania3_irq_type, handle_level_irq);
        else
            set_irq_chip_and_handler(i, &Titania3_fiq_type, handle_level_irq);
    }
    //disable all
    Titania3_DisableInterrupt(E_IRQ_FIQ_ALL);
#else
    ...
    set_irq_chip_and_handler(0, &Titania3_fiq_type, handle_level_irq);
    set_irq_chip_and_handler(1, &Titania3_fiq_type, handle_level_irq);
