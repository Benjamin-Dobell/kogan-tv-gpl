...
static int Uranus4_EnableInterrupt (InterruptNum eIntNum)
{
    int bRet = 0;
...
static int Uranus4_DisableInterrupt (InterruptNum eIntNum)
{
    if (eIntNum == E_IRQ_FIQ_ALL)
    {
        REG(REG_IRQ_MASK_L) |= IRQL_MASK;
...
static unsigned int startup_Uranus4_irq(unsigned int irq)
{
	//enable_lasat_irq(irq);
    Uranus4_EnableInterrupt((InterruptNum)irq);
	return 0; /* never anything pending */
}

static void shutdown_Uranus4_irq(unsigned int irq)
{
	//enable_lasat_irq(irq);
    Uranus4_DisableInterrupt((InterruptNum)irq);
	//return 0; /* never anything pending */
}

#define enable_Uranus4_irq startup_Uranus4_irq
#define disable_Uranus4_irq shutdown_Uranus4_irq
#define mask_and_ack_Uranus4_irq shutdown_Uranus4_irq
#define end_Uranus4_irq startup_Uranus4_irq

static unsigned int startup_Uranus4_fiq(unsigned int irq)
{
	//enable_lasat_irq(irq);
    Uranus4_EnableInterrupt((InterruptNum)irq);
	return 0; /* never anything pending */
...
static void shutdown_Uranus4_fiq(unsigned int irq)
{
	//enable_lasat_irq(irq);
    Uranus4_DisableInterrupt((InterruptNum)irq);
	//return 0; /* never anything pending */
...
#define enable_Uranus4_fiq startup_Uranus4_fiq
#define disable_Uranus4_fiq shutdown_Uranus4_fiq
#define mask_and_ack_Uranus4_fiq shutdown_Uranus4_fiq
#define end_Uranus4_fiq startup_Uranus4_fiq

// Dean modify to match kernel 2.6.26
static struct hw_interrupt_type Uranus4_irq_type = {
	.name = "Uranus4 IRQ",
	.startup = startup_Uranus4_irq,
	.shutdown = shutdown_Uranus4_irq,
	.enable = (void (*)(unsigned int irq))enable_Uranus4_irq,
	.disable = disable_Uranus4_irq,
	.ack = mask_and_ack_Uranus4_irq,
	.mask = disable_Uranus4_irq,
	.mask_ack = disable_Uranus4_irq,
	.unmask = (void (*)(unsigned int irq))enable_Uranus4_irq,
	.eoi = (void (*)(unsigned int irq))enable_Uranus4_irq,
	.end = (void (*)(unsigned int irq))end_Uranus4_irq
};
static struct hw_interrupt_type Uranus4_fiq_type = {
	.name = "Uranus4 IRQ",
	.startup = startup_Uranus4_fiq,
	.shutdown = shutdown_Uranus4_fiq,
	.enable = (void (*)(unsigned int irq))enable_Uranus4_fiq,
	.disable = disable_Uranus4_fiq,
	.ack = mask_and_ack_Uranus4_fiq,
	.mask = disable_Uranus4_fiq,
	.mask_ack = disable_Uranus4_fiq,
	.unmask = (void (*)(unsigned int irq))enable_Uranus4_fiq,
	.eoi = (void (*)(unsigned int irq))enable_Uranus4_fiq,
	.end = (void (*)(unsigned int irq))end_Uranus4_fiq
};

// void Uranus4_hw0_irqdispatch(struct pt_regs *regs)
void Uranus4_hw0_irqdispatch(void)
{
...
// void Uranus4_hw0_fiqdispatch(struct pt_regs *regs)
void Uranus4_hw0_fiqdispatch(void)
{
    unsigned int        u16Reg = 0;
    unsigned int        u32Bit;
    InterruptNum        eIntNum;
...
asmlinkage void plat_irq_dispatch(void)
{
	...
	irq = irq_ffs(pending);
       ...
       //if(irq != 7)
            //printk("\nirq = %d\n", irq);
       if (irq == 2)
              Uranus4_hw0_irqdispatch();
       else if (irq == 3)
              Uranus4_hw0_fiqdispatch();
	else if (irq >= 0)
...
void __init arch_init_irq(void)
{
	...
	//mips_cpu_irq_init(0);
#if 0
	for (i = 0; i <= Uranus4INT_END; i++) {
		irq_desc[i].status	= IRQ_DISABLED;
		irq_desc[i].action	= 0;
		irq_desc[i].depth	= 1;
		if ( i <64)
                irq_desc[i].chip    = &Uranus4_irq_type;
		   //irq_desc[i].handler	= &Uranus4_irq_type;
		else
                irq_desc[i].chip    = &Uranus4_fiq_type;
		   //irq_desc[i].handler  = &Uranus4_fiq_type;
	}

#endif
       for (i = MSTAR_INT_BASE; i <= (URANUS4INT_END+MSTAR_INT_BASE); i++) {
            if ( i <64+MSTAR_INT_BASE)
                set_irq_chip_and_handler(i, &Uranus4_irq_type, handle_level_irq);
            else
                set_irq_chip_and_handler(i, &Uranus4_fiq_type, handle_level_irq);
       }
       set_irq_chip_and_handler(0, &Uranus4_fiq_type, handle_level_irq);
       set_irq_chip_and_handler(1, &Uranus4_fiq_type, handle_level_irq);
	//disable all
	Uranus4_DisableInterrupt(E_IRQ_FIQ_ALL);
