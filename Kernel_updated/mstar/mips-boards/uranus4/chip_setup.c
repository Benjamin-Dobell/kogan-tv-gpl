...
const char display_string[] = "        LINUX ON Uranus4     ";
...
#define UART_SEL_PM51_UART0     0x06
        ...
        #define URANUS4_UART_CLK_FREQ     123000000
    #else//USE_ASIC_WITH_ICE
        #define URANUS4_UART_CLK_FREQ     12000000
    #endif//USE_ASIC_WITH_ICE
#endif

#if defined(CONFIG_MSTAR_URANUS4_BD_GENERIC)
#define URANUS4_UART_CLK_FREQ     123000000
#endif
...
static int __init Uranus4_setup(void)
{
	...
	printk("Begin Uranus4_setup\n");

#if defined(CONFIG_MSTAR_URANUS4)

#if defined(CONFIG_MSTAR_URANUS4_MMAP_32MB_32MB) || \
    defined(CONFIG_MSTAR_URANUS4_MMAP_64MB_32MB) || \
    defined(CONFIG_MSTAR_URANUS4_MMAP_64MB_64MB) || \
    defined(CONFIG_MSTAR_URANUS4_MMAP_128MB_64MB) || \
    defined(CONFIG_MSTAR_MMAP_128MB_128MB_DEFAULT) || \
    defined(CONFIG_MSTAR_URANUS4_MMAP_128MB_128MB)
...
static void __init serial_init(void)
{
	...
	// for FPGA
	s.uartclk   = URANUS4_UART_CLK_FREQ;//FOR_FPGA_UART
	//s.uartclk = 14318180;
	//s.uartclk = 123000000;
	//s.uartclk = 144000000;
	//s.uartclk = 160000000;
...
#ifdef CONFIG_MSTAR_URANUS4_BD_FPGA
...
#define URANUS4_UART_BASE        0xbf201300
#if 0
#define URANUS4_UART_RX          (0*4 + URANUS4_UART_BASE)
#define URANUS4_UART_TX          (0*4 + URANUS4_UART_BASE)
#define URANUS4_UART_DLL         (0*4 + URANUS4_UART_BASE)
#define URANUS4_UART_DLM         (1*4 + URANUS4_UART_BASE)
#define URANUS4_UART_IER         (1*4 + URANUS4_UART_BASE)
#define URANUS4_UART_IIR         (2*4 + URANUS4_UART_BASE)
#define URANUS4_UART_FCR         (2*4 + URANUS4_UART_BASE)
#define URANUS4_UART_EFR         (2*4 + URANUS4_UART_BASE)
#define URANUS4_UART_LCR         (3*4 + URANUS4_UART_BASE)
#define URANUS4_UART_MCR         (4*4 + URANUS4_UART_BASE)
#define URANUS4_UART_LSR         (5*4 + URANUS4_UART_BASE)
#define URANUS4_UART_MSR         (6*4 + URANUS4_UART_BASE)
#define URANUS4_UART_SCR         (7*4 + URANUS4_UART_BASE)
#define URANUS4_UART_SCCR_RST    (8*4 + URANUS4_UART_BASE)
#else
#define URANUS4_UART_RX          (0x00*4 + URANUS4_UART_BASE)
#define URANUS4_UART_TX          (0x00*4 + URANUS4_UART_BASE)
#define URANUS4_UART_DLL         (0x00*4 + URANUS4_UART_BASE)
#define URANUS4_UART_DLM         (0x02*4 + URANUS4_UART_BASE)
#define URANUS4_UART_IER         (0x02*4 + URANUS4_UART_BASE)
#define URANUS4_UART_IIR         (0x04*4 + URANUS4_UART_BASE)
#define URANUS4_UART_FCR         (0x04*4 + URANUS4_UART_BASE)

#define URANUS4_UART_LCR         (0x06*4 + URANUS4_UART_BASE)
#define URANUS4_UART_MCR         (0x08*4 + URANUS4_UART_BASE)
#define URANUS4_UART_LSR         (0x0A*4 + URANUS4_UART_BASE)
#define URANUS4_UART_MSR         (0x0C*4 + URANUS4_UART_BASE)
#endif
...
#ifdef CONFIG_MSTAR_URANUS4_BD_FPGA
#if USE_ASIC_WITH_ICE
    #define URANUS4_UART_BAUD_RATE  115200
#else//USE_ASIC_WITH_ICE
    #define URANUS4_UART_BAUD_RATE  38400
#endif//USE_ASIC_WITH_ICE
#else
    #define URANUS4_UART_BAUD_RATE  115200
#endif
...
//Divisor
#define URANUS4_UART_DIVISOR    (URANUS4_UART_CLK_FREQ/(16 * URANUS4_UART_BAUD_RATE))
...
int uranus4_putc(int c)
{//jc_db: n2r
    unsigned char u8Reg;

    do {
        u8Reg = UART_REG(URANUS4_UART_LSR)&0xff;
    ...
    if(c=='\n')
    {
        ...
        UART_REG(URANUS4_UART_TX)='\r';
    }
    else
    {
        UART_REG(URANUS4_UART_TX)=c&0xffff;
    }
    do {
        u8Reg = UART_REG(URANUS4_UART_LSR)&0xff;
        if ((u8Reg & UART_LSR_THRE) == UART_LSR_THRE)
...
void uart_clk_scan(void)
{
    ...
    {
        unsigned short divisor= (clk[i]/(16 * URANUS4_UART_BAUD_RATE));
        //UART1 INIT:

        //Disable all UART interrupts
        UART_REG(URANUS4_UART_IER) = 0x00;

        //Reset receiver and transmitr
        UART_REG(URANUS4_UART_FCR) = 0x07;

        //Set 8 bit char, 1 stop bit, no parity
        UART_REG(URANUS4_UART_LCR) = 0x03;

        UART_REG(URANUS4_UART_LCR) = 0x83;
        UART_REG(URANUS4_UART_DLM) = (divisor >> 8) & 0x00ff;
        UART_REG(URANUS4_UART_DLL) = divisor & 0x00ff;
        UART_REG(URANUS4_UART_LCR) = 0x03;

        UART_REG(URANUS4_UART_TX) = '0'+ i;
    }
}
...
void pool_getc (void)
{
    ...
    nflag = 0 ;
    do {
        if ( ((UART_REG(URANUS4_UART_LSR) & UART_LSR_DR) == UART_LSR_DR) || nflag )
        {
            break;
        }
    } while (1);
    u8Ch = (char)UART_REG(URANUS4_UART_RX);      //get char
    uranus4_putc(u8Ch);
    return ;
}
...
#if  defined(CONFIG_MSTAR_URANUS4_BD_FPGA) || !(USE_ASIC_WITH_ICE)
    //FOR_FPGA_UART
...
#else//CONFIG_MSTAR_URANUS4_BD_FPGA
...
//// Switch PAD RS232 end///////////////////////////////////////////////////
    ...
    UART_REG(URANUS4_UART_LCR) |= 0x80;
    UART_REG(URANUS4_UART_DLM) = (URANUS4_UART_DIVISOR >> 8) & 0x00ff;
    UART_REG(URANUS4_UART_DLL) = URANUS4_UART_DIVISOR & 0x00ff;
    UART_REG(URANUS4_UART_LCR) &= ~(0x80) ;

    // Enable receiver data available interrupt
    //UART_REG(URANUS4_UART_IER) = 0x01;

    //UART_REG(URANUS4_UART_LCR) &= ~(0x8000);

    //UART_REG(URANUS4_UART_SCCR_RST) = 0x10 ;
    ...
    uranus4_putc('T');
    uranus4_putc('\n');
    //UART_REG(URANUS4_UART_MCR) |= 0x10 ;
...
unsigned int Chip_Query_MIPS_CLK(void)
{
    ...
    else if(uSpeed_H ==0x14)////// reduce the mips to 480MHZ
    {
        u32Count = 480000000;
    }
    else if( uSpeed_H ==0x15 )
    {
        u32Count = 504000000;
    }
    else if( uSpeed_H ==0x16 )
    {
        u32Count = 528000000;
    }
    ...
    else if( uSpeed_H ==0x20 )
    {
        u32Count = 624000000;
    }
    else if( uSpeed_H ==0x21 )
    {
        u32Count = 648000000;
    }
#endif
