...
static int Titania10_EnableInterrupt (InterruptNum eIntNum)
{
    int bRet = 0;
...
static int Titania10_DisableInterrupt (InterruptNum eIntNum)
{
...
static unsigned int startup_Titania10_irq(unsigned int irq)
{
	//enable_lasat_irq(irq);
    Titania10_EnableInterrupt((InterruptNum)irq);
	return 0; /* never anything pending */
}

static void shutdown_Titania10_irq(unsigned int irq)
{
	//enable_lasat_irq(irq);
    Titania10_DisableInterrupt((InterruptNum)irq);
	//return 0; /* never anything pending */
}

#define enable_Titania10_irq startup_Titania10_irq
#define disable_Titania10_irq shutdown_Titania10_irq
#define mask_and_ack_Titania10_irq shutdown_Titania10_irq
#define end_Titania10_irq startup_Titania10_irq

static unsigned int startup_Titania10_fiq(unsigned int irq)
{
	//enable_lasat_irq(irq);
    Titania10_EnableInterrupt((InterruptNum)irq);
	return 0; /* never anything pending */
...
static void shutdown_Titania10_fiq(unsigned int irq)
{
	//enable_lasat_irq(irq);
    Titania10_DisableInterrupt((InterruptNum)irq);
	//return 0; /* never anything pending */
...
#define enable_Titania10_fiq startup_Titania10_fiq
#define disable_Titania10_fiq shutdown_Titania10_fiq
#define mask_and_ack_Titania10_fiq shutdown_Titania10_fiq
#define end_Titania10_fiq startup_Titania10_fiq

// Dean modify to match kernel 2.6.26
static struct hw_interrupt_type Titania10_irq_type = {
	.name = "Titania10 IRQ",
	.startup = startup_Titania10_irq,
	.shutdown = shutdown_Titania10_irq,
	.enable = (void (*)(unsigned int irq))enable_Titania10_irq,
	.disable = disable_Titania10_irq,
	.ack = mask_and_ack_Titania10_irq,
	.mask = disable_Titania10_irq,
	.mask_ack = disable_Titania10_irq,
	.unmask = (void (*)(unsigned int irq))enable_Titania10_irq,
	.eoi = (void (*)(unsigned int irq))enable_Titania10_irq,
	.end = (void (*)(unsigned int irq))end_Titania10_irq
};
static struct hw_interrupt_type Titania10_fiq_type = {
	.name = "Titania10 IRQ",
	.startup = startup_Titania10_fiq,
	.shutdown = shutdown_Titania10_fiq,
	.enable = (void (*)(unsigned int irq))enable_Titania10_fiq,
	.disable = disable_Titania10_fiq,
	.ack = mask_and_ack_Titania10_fiq,
	.mask = disable_Titania10_fiq,
	.mask_ack = disable_Titania10_fiq,
	.unmask = (void (*)(unsigned int irq))enable_Titania10_fiq,
	.eoi = (void (*)(unsigned int irq))enable_Titania10_fiq,
	.end = (void (*)(unsigned int irq))end_Titania10_fiq
};

// void Titania10_hw0_irqdispatch(struct pt_regs *regs)
void Titania10_hw0_irqdispatch(void)
{
...
// void Titania10_hw0_fiqdispatch(struct pt_regs *regs)
void Titania10_hw0_fiqdispatch(void)
{
    unsigned int        u16Reg = 0;
    unsigned int        u32Bit;
    InterruptNum        eIntNum;
...
asmlinkage void plat_irq_dispatch(void)
{
	...
	irq = irq_ffs(pending);
       ...
       //if(irq != 7)
            //printk("\nirq = %d\n", irq);
       if (irq == 2)
              Titania10_hw0_irqdispatch();
       else if (irq == 3)
              Titania10_hw0_fiqdispatch();
	else if (irq >= 0)
...
void __init arch_init_irq(void)
{
	...
	//mips_cpu_irq_init(0);
#if 0
	for (i = 0; i <= TITANIA10INT_END; i++) {
		irq_desc[i].status	= IRQ_DISABLED;
		irq_desc[i].action	= 0;
		irq_desc[i].depth	= 1;
		if ( i <64)
                irq_desc[i].chip    = &Titania10_irq_type;
		   //irq_desc[i].handler	= &Titania10_irq_type;
		else
                irq_desc[i].chip    = &Titania10_fiq_type;
		   //irq_desc[i].handler  = &Titania10_fiq_type;
	}
    ...
    if (cpu_has_vint) {
        ...
        set_vi_handler(2, Titania10_hw0_irqdispatch);
        set_vi_handler(3, Titania10_hw0_fiqdispatch);
    ...
    for (i = MSTAR_INT_BASE; i <= (TITANIA10INT_END+MSTAR_INT_BASE); i++) {
        if ( i <64+MSTAR_INT_BASE)
            set_irq_chip_and_handler(i, &Titania10_irq_type, handle_level_irq);
        else
            set_irq_chip_and_handler(i, &Titania10_fiq_type, handle_level_irq);
    }
    //disable all
    Titania10_DisableInterrupt(E_IRQ_FIQ_ALL);
#else
    ...
    set_irq_chip_and_handler(0, &Titania10_fiq_type, handle_level_irq);
    set_irq_chip_and_handler(1, &Titania10_fiq_type, handle_level_irq);
