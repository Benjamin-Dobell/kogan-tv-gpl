...
#if defined(CONFIG_MSTAR_TITANIA8_BD_GENERIC)  || defined(CONFIG_MSTAR_TITANIA12_BD_GENERIC)
#define MSTAR_CHIP_UART_CLK_FREQ     123000000
#endif
...
#if defined(CONFIG_MSTAR_TITANIA8_MMAP_32MB_32MB) || \
    defined(CONFIG_MSTAR_TITANIA8_MMAP_64MB_32MB) || \
    defined(CONFIG_MSTAR_TITANIA8_MMAP_64MB_64MB) || \
    defined(CONFIG_MSTAR_TITANIA8_MMAP_128MB_64MB) || \
    defined(CONFIG_MSTAR_MMAP_128MB_128MB_DEFAULT) || \
    defined(CONFIG_MSTAR_TITANIA8_MMAP_128MB_128MB) || \
    defined(CONFIG_MSTAR_TITANIA8_MMAP_256MB_256MB)  || \
    defined(CONFIG_MSTAR_TITANIA8_MMAP_512MB_512MB) || \
    defined(CONFIG_MSTAR_TITANIA12_MMAP_32MB_32MB) || \
    defined(CONFIG_MSTAR_TITANIA12_MMAP_64MB_32MB) || \
    defined(CONFIG_MSTAR_TITANIA12_MMAP_64MB_64MB) || \
    defined(CONFIG_MSTAR_TITANIA12_MMAP_128MB_64MB) || \
    defined(CONFIG_MSTAR_MMAP_128MB_128MB_DEFAULT) || \
    defined(CONFIG_MSTAR_TITANIA12_MMAP_128MB_128MB) || \
    defined(CONFIG_MSTAR_TITANIA12_MMAP_256MB_256MB)  || \
    defined(CONFIG_MSTAR_TITANIA12_MMAP_512MB_512MB) || \
...
#ifdef CONFIG_MSTAR_TITANIA8_BD_FPGA
...
#ifdef CONFIG_MSTAR_TITANIA12_BD_FPGA
        s.uartclk = 12000000 ;
#else
        s.uartclk = 123000000;
#endif
...
{
//#ifdef  CONFIG_MSTAR_TITANIA8_BD_FPGA || CONFIG_MSTAR_TITANIA12_BD_FPGA
#if 0
    //FOR_FPGA_UART
...
///// Switch PAD ISP//////////////////////////////////////////////////////////
    ...
    //ori     $11, $11,   0x0004
    //sw      $11,        0xbf203d4c
    //*(volatile volatile unsigned int*)(0xbf203d4c) &= ~0x000F; //uart mux has been set at MBoot
...
inline void _chip_flush_miu_pipe(void)
{
    ...
    *(volatile unsigned int *)(0xbf000000+0x203104) = _BIT(4) | 0x2; // H41 all
    ...
    do
    {
        tmp = *(volatile unsigned int *)(0xbf000000+0x203104);
        tmp &= _BIT(12);//pipe
    }while(tmp == 0);
    spin_unlock_irqrestore(&l2prefetch_lock, flag);
#endif
...
void Chip_L2_cache_wback_inv( unsigned long addr, unsigned long size)
{
    ...
    else if( size >= MIPS_L2_WB_INV_TH )
    {
        spin_lock_irqsave(&l2prefetch_lock, flag);
    ...
    else // range flush
    {
        ...
        *(volatile unsigned int *)(0xbf000000+0x203104) = _BIT(4) | 0x6; // H41 all
        ...
        /**(volatile unsigned int *)(0xbf000000+0x203104) = 0x7; // H41 all
        *(volatile unsigned int *)(0xbf000000+0x203104) = _BIT(4) | 0x7; // H41 all
        do
        {
            temp = *(volatile unsigned int *)(0xbf000000+0x203104);
            temp &= _BIT(11);//all
...
void Chip_L2_cache_inv( unsigned long addr, unsigned long size)
{
    ...
    if( u32L2CacheEnabled ==0 )
    {
        ...
        _chip_flush_miu_pipe();
        #if 0
        *(volatile unsigned int *)(0xbf000000+0x203100) = 0x40004000; //H40 BIT14=1
        do
        {
            temp = *(volatile unsigned int *)(0xbf000000+0x203100);
            temp &= _BIT(14);
        }while(temp == 0);
        *(volatile unsigned int *)(0xbf000000+0x203100) = 0x40000000; //H40 BIT14=0
        #endif
#endif
    }
    else if( size >= MIPS_L2_INV_TH )
    {
        spin_lock_irqsave(&l2prefetch_lock, flag);
        *(volatile unsigned int *)(0xbf000000+0x203104) =  0x4; // H41 all
        *(volatile unsigned int *)(0xbf000000+0x203104) = _BIT(4) | 0x4; // H41 all
...
unsigned int Chip_Query_MIPS_CLK(void)
{
    ...
    if( Chip_Query_Rev()  >= 0 )
    {
        ...
        if(  ((*((volatile u32*)0xBF22184C)) & _BIT(8)) == 0 )
        {
            u32Count=u32Count>>1;
        }
    }
...
#elif defined(CONFIG_MSTAR_TITANIA9) || defined(CONFIG_MSTAR_TITANIA13)
    if( mphyaddr < HIGH_MEM_PHY_BASE )
    {
        return (void*)(mphyaddr|0x80000000);
    }
