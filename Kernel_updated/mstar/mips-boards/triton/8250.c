...
static int keyidx = 0 ;
static int control_state = 0 ; // 0=normal, 1=silent
...
static void UART16550_WRITE(u8 addr, u8 data)
{
    ...
    if( addr==3 ){ // IER
        UART_REG_ONE_BYTE(5) = data ;
        return ;
    }

    if( 1==control_state )
        return ;
    UART_REG(addr>>1) = new_data;
}
...
static void __init serial8250_isa_init_ports(void)
{
	...
	for (i = 0; i < UART_NR; i++) {
		...
		up->tx_loadsz = 8 ; // use the 8 byte depth FIFO well
	}
