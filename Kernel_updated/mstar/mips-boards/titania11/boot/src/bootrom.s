...
Dcache_loop:
    ...
    add     $14, $14, 1
    sub     $15, 1
    bnez    $15, Dcache_loop    # loop until Set = 0
    nop
...
BOOTROM_Entry:
    ...
    DBGREG  0x0010  #JC: if boot fail in FPGA, maybe you can re-program SOF file. boot to busybox need 4min
...
.if 1
    ...
    # UART_SEL0 --> PIU UART0
    RBASE 0xbf203D4C
    WREG  0xbf203D4C, 0x3214

    # UART mode
    WREG  0xbf203DB8, 0x0000

    #enable reg_uart_rx_enable;
    #WREG  0xbf203C04, 0x0400

    RBASE  UART_IER
    WREG   UART_IER, 0x0000
    WREG   UART_FCR, 0x0007
    WREG   UART_LCR, 0x0003

    # /* Set baud rate */
    WREG   UART_LCR, 0x0083
    WREG   UART_DLM, ((UART_DIVISOR >> 8) & 0xff)
    WREG   UART_DLL, (UART_DIVISOR & 0xff)
    WREG   UART_LCR, 0x0003
.else
    RBASE  0xbf003d54
    WREG   0xbf003d54, 0x0014   #  select UART0 to MIPS
    ...
    WREG   0xbf201300,UART_DIVISOR
    WREG   0xbf201304,(UART_DIVISOR >> 8)
...
.if BD_FPGA
    RBASE 0xbf006010
    WREG  0xbf006010, 0x0000                  # watch dog -> MCU  55aa->aa55
    WREG  0xbf006014, 0x0000                  # watch dog -> MCU  55aa->aa55
    RBASE 0xbf203d40
    WREG  0xbf203d40, 0x0000                  # 2009/1/14 Julian add
    WREG  0xbf202404, 0x0000
    WREG  0xbf202404, 0x0818                  # initial
    WREG  0xbf202408, 0x0185                  # initial
...
.else #(.if BD_FPGA)
    ...
    RBASE 0xbf001c80
    WREG 0xbf001c80, 0x0002  # MCU clock -> 172MHz
    WREG 0xbf001c80, 0x0082  # MCU clock -> 172MHz
