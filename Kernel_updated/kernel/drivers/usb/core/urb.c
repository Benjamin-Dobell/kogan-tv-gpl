...
#if defined(HOTPLUG) && (MP_USB_MSTAR==1)	//tony add for hotplug (FS)
...
//	wait_event(usb_kill_urb_queue, atomic_read(&urb->use_count) <= 0); //120314, modify for wifi load/unload repeatly
	wait_event_timeout(usb_kill_urb_queue,
			atomic_read(&urb->use_count) <= 0, msecs_to_jiffies(2000)); //120314, modify for wifi load/unload repeatly
#else
