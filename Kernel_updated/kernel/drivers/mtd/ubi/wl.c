				...
				int cancel)
{
	...
	if (!ubi->scrub.rb_node) {
		...
		e2 = find_wl_entry(&ubi->free, WL_FREE_MAX_DIFF);
		paranoid_check_in_wl_tree(ubi, e1, &ubi->scrubbad);
		...
		e2 = find_wl_entry(&ubi->free, WL_FREE_MAX_DIFF);
		paranoid_check_in_wl_tree(ubi, e1, &ubi->scrubtorture);
...
#if defined(CONFIG_MTD_UBI_BITFLIPS) && (MP_NAND_UBI == 1)
	if(scrubbing == 2)
		err = schedule_erase(ubi, e1, 1);
	else if (scrubbing == 3)
		err = schedule_erase(ubi, e1, 2);
	else 
#endif
...
retry:
	...
	if (e == ubi->move_from) {
		...
		if (in_wl_tree(e, &ubi->used)) {
			...
			paranoid_check_in_wl_tree(ubi, e, &ubi->scrubbad);
			rb_erase(&e->u.rb, &ubi->scrubbad);
		} else if (in_wl_tree(e, &ubi->scrubtorture)) {
			paranoid_check_in_wl_tree(ubi, e, &ubi->scrubtorture);
