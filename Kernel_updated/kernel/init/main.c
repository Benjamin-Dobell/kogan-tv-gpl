...
static noinline void __init_refok rest_init(void)
{
	...
	#if (MP_JIFFIES_PRINT == 1)
	printk("before forking init thread, kernel takes about %d ms\n", jiffies*1000/CONFIG_HZ);
	#endif
	/*
...
static noinline int init_post(void)
{
	...
	if (execute_command) {
		...
		unsigned int PiuTick = reg_readw(0x1f006090);
...
{
	#if (MP_WDT_OFF_DBG == 1)
		#if defined(CONFIG_MSTAR_KERNEL_PANIC_DO_WDT_RESET) || defined(CONFIG_MSTAR_KERNEL_FAULT_DO_WDT_RESET)
		    if( Kernel_Level() != 0 ) // disable WDT if debug mode
		    {
        		*(volatile int*)(0xFD000000+(0x300A<<1)) = 0x0;
		        *(volatile int*)(0xFD000000+(0x3008<<1)) = 0x0;
    		    }
		#endif
	#endif
	/*
