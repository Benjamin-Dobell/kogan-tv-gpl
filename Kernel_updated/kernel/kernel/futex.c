...
extern int kdbg_futex_config_wait_msec;
...
static void wake_futex(struct futex_q *q)
{
	...
	if (kdbg_futex_dbg_start &&
		...
		/*
		 * Check if it is the target pid
		 * Check if the condition is met..
		 */
		unsigned int wait_jiffies = jiffies - q->timestamp;
		unsigned int wait_time_msec = jiffies_to_msecs(wait_jiffies);

		dprintk("Target pid: %d, current pid: %d\n",
				kdbg_futex_dbg_pid, p->pid);
		printk("waited for %u msec\n", wait_time_msec);
		/* if condition is met */
		if (wait_time_msec < kdbg_futex_config_wait_msec) {
